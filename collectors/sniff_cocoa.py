...
def got_key_press(keycode, modifiers, string, is_repeat):
    #modifiers.append(string)
    #print "<%s>" % "+".join(modifiers)
    pass

def got_mouse_click(button, x, y):
    #print "Button pressed: %d\nAt: (%d|%d)" % (button, x, y)
    pass

def got_screen_change(process_name, window_name, win_x, win_y, width, height):
    pass
...
def main():
    ...
    s.key_hook = got_key_press
    s.mouse_button_hook = got_mouse_click
    s.mouse_move_hook = got_mouse_move
    s.screen_hook = got_screen_change

    s.run()

class ActivitySniffer:
    def __init__(self):
    ...
    def handler(self, event):
        try:
            activeApps = filter(lambda x: x.isActive(), self.workspace.runningApplications())
            ...
            for app in activeApps:
                ...
                for window in windowList:
                    if (window['kCGWindowNumber'] == event.windowNumber()
                        or (not event.windowNumber() and window['kCGWindowOwnerName'] == app.localizedName())):
